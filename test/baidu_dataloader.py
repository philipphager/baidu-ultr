"""
Original data loading from the Baidu WSDM 23 cup:
https://github.com/ChuXiaokai/baidu_ultr_dataset/blob/main/dataloader.py
"""
import gzip
import math
import os
...
def mask_data(seq, mask_ignore_token_ids=[config["CLS"], config["SEP"], config["PAD"]],
              mask_token_id=config["MASK"],
              mask_prob=0.1,
              pad_token_id=config["PAD"],
              replace_prob=1.0
              ):
...
def process_data(query, title, content, max_seq_len):
    ...
    if len(data) < max_seq_len:
        ...
        data += [config["PAD"]] * (max_seq_len - len(data))
    else:
            ...
            with gzip.open(os.path.join(self.directory_path, file), 'rb') as f:
                for line in f.readlines():
                    ...
                    if len(line_list) == 3:  # new query
                        ...
                        position, title, content, click_label = line_list[0], line_list[
                            2], line_list[3], line_list[5]
                        try:
                            src_input, segment, src_padding_mask = process_data(
                                self.cur_query, title, content, self.max_seq_len)
                            buffer.append([src_input, segment, src_padding_mask,
                                           float(click_label)])
                        except:
                            pass
...
class TestDataset(Dataset):
    ...
    def load_annotate_data(self, fpath):
        ...
        for line in open(fpath, 'rb'):
            ...
            src_input, src_segment, src_padding_mask = process_data(query, title,
                                                                    content,
                                                                    self.max_seq_len)
        ...
        with gzip.open(fpath, 'rb') as f:
            ...
            for line in f.readlines():
                ...
                if len(line_list) == 3:  # new query
                    ...
                    position, title, content, click_label = line_list[0], line_list[2], \
